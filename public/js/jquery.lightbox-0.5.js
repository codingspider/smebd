...
(function($) {
	...
	$.fn.lightBox = function(settings) {
		...
		settings = jQuery.extend({
			...
			imageLoading:			'http://dpp.ocpl.com.bd/images/lightbox-ico-loading.gif',		// (string) Path and the name of the loading icon
			imageBtnPrev:			'http://dpp.ocpl.com.bd/images/left.png',			// (string) Path and the name of the prev button image
			imageBtnNext:			'http://dpp.ocpl.com.bd/images/right.png',			// (string) Path and the name of the next button image
			imageBtnClose:			'http://dpp.ocpl.com.bd/images/lightbox-btn-close.gif',		// (string) Path and the name of the close btn
		...
		function _set_navigation() {
			...
			if ( settings.activeImage != 0 ) {
				if ( settings.fixedNavigation ) {
					$('#lightbox-nav-btnPrev').css({ 'background' : 'url(' + settings.imageBtnPrev + ') left 49% no-repeat' })
						.unbind()
						.bind('click',function() {
					...
					$('#lightbox-nav-btnPrev').unbind().hover(function() {
						$(this).css({ 'background' : 'url(' + settings.imageBtnPrev + ') left 49% no-repeat' });
					},function() {
			...
			if ( settings.activeImage != ( settings.imageArray.length -1 ) ) {
				if ( settings.fixedNavigation ) {
					$('#lightbox-nav-btnNext').css({ 'background' : 'url(' + settings.imageBtnNext + ') right 49% no-repeat' })
						.unbind()
						.bind('click',function() {
							settings.activeImage = settings.activeImage + 1;
							_set_image_to_view();
					...
					$('#lightbox-nav-btnNext').unbind().hover(function() {
						$(this).css({ 'background' : 'url(' + settings.imageBtnNext + ') right 49% no-repeat' });
					},function() {
						$(this).css({ 'background' : 'transparent url(' + settings.imageBlank + ') no-repeat' });
					}).show().bind('click',function() {
						settings.activeImage = settings.activeImage + 1;
